...
import ClinicalNotesSearch from "@/components/clinical-notes/clinical-notes-search"
...
export default function ClinicalNotesClient() {
  ...
  const [careProfessionals, setCareProfessionals] = useState<any[]>([])

  useEffect(() => {
    const fetchData = async () => {
      setIsLoading(true)
      try {
        const [notesRes, categoriesRes, templatesRes, professionalsRes] = await Promise.all([
          fetch("/api/clinical-notes"),
          fetch("/api/clinical-notes/categories"),
          fetch("/api/clinical-notes/templates"),
          fetch("/api/care-professionals"),
        ])

        if (!notesRes.ok || !categoriesRes.ok || !templatesRes.ok || !professionalsRes.ok) {
          throw new Error("Failed to fetch data")
        }

        const [notesData, categoriesData, templatesData, professionalsData] = await Promise.all([
          notesRes.json(),
          categoriesRes.json(),
          templatesRes.json(),
        ...
        setCareProfessionals(professionalsData)
      } catch (error) {
  ...
  const handleSearch = async (filters: any) => {
    ...
    try {
      ...
      const response = await fetch(`/api/clinical-notes/search?${params}`)
      ...
      toast({
        title: "Error",
        description: "Failed to search clinical notes",
        variant: "destructive",
      })
    } finally {
      setIsLoading(false)
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
      ...
      <div className="md:col-span-3">
        <ClinicalNotesSearch categories={categories} careProfessionals={careProfessionals} onSearch={handleSearch} />
