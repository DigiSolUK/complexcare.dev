...
export async function getById<T = any>(table: string, id: string, tenantId: string): Promise<T | null> {
...
export async function insert<T = any>(table: string, data: any, tenantId: string): Promise<T | null> {
  const results = await clientDb<T>("insert", { table, data: { ...data, tenant_id: tenantId }, tenantId })
  return results[0] || null
}

export async function update<T = any>(table: string, id: string, data: any, tenantId: string): Promise<T | null> {
  const results = await clientDb<T>("update", { table, id, data, tenantId })
  return results[0] || null
}

export async function remove(table: string, id: string, tenantId: string): Promise<boolean> {
