...
export async function createClinicalNote(
  ...
  try {
    // Log activity for creating clinical note
    await logActivity({
      tenantId,
      activityType: "clinical_note_created",
      description: `Clinical note created for patient`,
...
export async function updateClinicalNote(
  id: string,
  data: Partial<
    Omit<ClinicalNote, "id" | "created_at" | "updated_at" | "category_name" | "category_color" | "created_by_name">
  >,
  tenantId: string = DEFAULT_TENANT_ID,
): Promise<ClinicalNote | null> {
  try {
    ...
    // Add each field that's present in the data object
    ...
    // If no fields to update, return the existing note
    ...
    if (result.rows && result.rows.length > 0) {
      // Log activity for updating clinical note
      await logActivity({
        tenantId,
...
export async function deleteClinicalNote(id: string, tenantId: string = DEFAULT_TENANT_ID): Promise<boolean> {
  try {
    // Get the note first to log the activity
    ...
    if (result.rows && result.rows.length > 0) {
      // Log activity for deleting clinical note
      await logActivity({
        tenantId,
...
export async function addAttachmentToNote(
  ...
  try {
    // Get the note to verify it exists and belongs to the tenant
    const note = await getClinicalNoteById(noteId, tenantId)

    if (!note) {
      throw new Error("Note not found or does not belong to the tenant")
    }
    ...
    if (result.rows && result.rows.length > 0) {
      ...
      await logActivity({
        ...
        description: `Attachment added to clinical note`,
        patientId: note.patient_id,
      })
