...
export const validatePatientsTable = async (tenantId: string) => {
  try {
    const result = await db.query("SELECT COUNT(*) FROM patients WHERE tenant_id = $1", [tenantId])
    return { valid: true, count: Number.parseInt(result.rows[0].count) }
  } catch (error) {
...
export const createTestPatient = async (tenantId: string) => {
  const testPatient = {
    firstName: "Test",
  ...
  return PatientService.createPatient(testPatient, tenantId)
}
