...
export async function getCareProfessionals(tenantId: string, searchQuery?: string): Promise<CareProfessional[]> {
  try {
    ...
    return rows.map(mapCareProfessionalRow)
  } catch (error) {
...
export async function getCareProfessionalById(id: string, tenantId: string): Promise<CareProfessional | null> {
  try {
    ...
    `
    return rows.length > 0 ? mapCareProfessionalRow(rows[0]) : null
  } catch (error) {
  ...
  try {
    ...
    const { rows } = await sql`
      INSERT INTO care_professionals (
        ...
        ${id}, ${tenantId}, ${data.first_name}, ${data.last_name}, ${data.email}, ${data.role}, ${data.phone || null}, ${data.specialization || null},
        ${data.qualification || null}, ${data.license_number || null}, ${data.employment_status || null}, ${data.start_date || null}, ${data.is_active !== undefined ? data.is_active : true},
        ${data.address ? JSON.stringify(data.address) : null}::jsonb, ${data.notes || null}, ${data.emergency_contact_name || null}, ${data.emergency_contact_phone || null},
        ${data.avatar_url || null}, ${now}, ${now}, ${createdBy}, ${createdBy}
      )
      RETURNING *
    `
    return mapCareProfessionalRow(rows[0])
  } catch (error) {
...
export async function updateCareProfessional(
  ...
  try {
    ...
    if (dataToUpdate.start_date) {
      dataToUpdate.start_date = new Date(dataToUpdate.start_date).toISOString().split("T")[0]
    }

    const dataWithUpdater = { ...dataToUpdate, updated_by: updatedBy, updated_at: new Date().toISOString() }
