...
export async function getTasks(
  ...
  try {
    ...
    const tasks = await executeQuery<Task>(
      `SELECT * FROM tasks WHERE tenant_id = $1 ${filter.clause} ORDER BY created_at DESC`,
...
export async function getTaskById(id: string): Promise<Task | null> {
  try {
    const tasks = await executeQuery<Task>(`SELECT * FROM tasks WHERE id = $1 AND deleted_at IS NULL LIMIT 1`, [id])
...
export async function deleteTask(id: string): Promise<boolean> {
  try {
    const result = await executeQuery(`UPDATE tasks SET deleted_at = NOW() WHERE id = $1`, [id])
...
export async function getTasksByAssignee(
  ...
  try {
    const filter = buildTaskFilterClause(filters || {})
    const tasks = await executeQuery<Task>(
      `SELECT * FROM tasks WHERE assigned_to_id = $1 AND tenant_id = $2 ${filter.clause} ORDER BY created_at DESC`,
...
export async function getTasksByPatient(
  ...
  try {
    const filter = buildTaskFilterClause(filters || {})
    const tasks = await executeQuery<Task>(
      `SELECT * FROM tasks WHERE patient_id = $1 AND tenant_id = $2 ${filter.clause} ORDER BY created_at DESC`,
