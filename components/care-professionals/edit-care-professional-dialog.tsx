...
interface EditCareProfessionalDialogProps {
  isOpen: boolean
  onClose: () => void
  onCareProfessionalUpdated: () => void
  careProfessional: CareProfessional
}

export function EditCareProfessionalDialog({
  isOpen,
  onClose,
  onCareProfessionalUpdated,
  careProfessional,
}: EditCareProfessionalDialogProps) {
  ...
  useEffect(() => {
    if (careProfessional) {
      setFormData({
        ...
        email: careProfessional.email,
        phone: careProfessional.phone || "",
        role: careProfessional.role,
        specialization: careProfessional.specialization || "",
        qualification: careProfessional.qualification || "",
        license_number: careProfessional.license_number || "",
        employment_status: careProfessional.employment_status || "Full-time",
        start_date: new Date(careProfessional.start_date),
        is_active: careProfessional.is_active,
        address:
          typeof careProfessional.address === "object" && careProfessional.address !== null
            ? JSON.stringify(careProfessional.address)
            : careProfessional.address || "",
        notes: careProfessional.notes || "",
        emergency_contact_name: careProfessional.emergency_contact_name || "",
        emergency_contact_phone: careProfessional.emergency_contact_phone || "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`/api/care-professionals/${careProfessional.id}`, {
        method: "PUT",
        headers: {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Edit Care Professional</DialogTitle>
        </DialogHeader>
